...
**Note**:  You do not have go through these materials in detail. They only serves as a reference. We have provided sufficient scaffolding code for you. You only need to modify the relevant part that handles stdin and stdout. The goal of this assignment is to give your experience with writing socket programs in C and Python, so that you know what to do in the future if you are asked to write programs that require communication between computers.
